...
const StatusPage = () => {
  ...
  const addRow = async () => {
    try {
      ...
      numeInput.className = 'border border-gray-300 rounded px-2 py-1 bg-gray-100';

      newRow.appendChild(createCell(numeInput));

      const tipStatusInput = document.createElement('select');
      tipStatusInput.className = 'border border-gray-300 rounded px-2 py-1 bg-gray-100';
      ...
      statusDesignInput.className = 'border border-gray-300 rounded px-2 py-1 bg-gray-100';
      ...
      actionButton.className = "text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800";
  ...
  function add_status_to_db() {
    ...
    const statusDesignInput = document.querySelector('input[name="statusDesign"]');
  ...
  async function add_query(statusData) {
    try {
      ...
      throw new Error(`Error updating status in the database: ${error.message}`);
    }
  }


  const editRow = async (statusId, statusName, tipStatus, statusDesign, index) => {
            ...
            <table className="min-w-max w-full table-auto">
              ...
              <tbody className="text-gray-600 text-sm font-light">
                {statuses.map((status, index) => (
                  <tr className="border-b border-gray-200 hover:bg-gray-100" key={status.Id} id={`${index + 1}`}>
                    <td className="py-3 px-6 text-left whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span name="continut-nume" className="font-medium">{status.nume}</span>
                      </div>
                    </td>
                    <td className="py-3 px-6 text-left">
                      <div className="flex items-center">
                        ...
                        <span name="continut-tipstatus">{status.TipStatus}</span>
                      </div>
                    </td>
                    <td className="py-3 px-6 text-center">
                      <div className="flex items-center justify-center">
                        <span name="continut-statusdesign">{status.StatusDesign}</span>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-3 text-center">
                      <div className="flex item-center justify-center">
                        <div className="w-4 mr-2 transform hover:text-green-500 hover:scale-110">
                          <svg onClick={() => editRow(status.Id, status.nume, status.TipStatus, status.StatusDesign, index)}
                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                          </svg>
                        </div>
                        <div className="w-4 mr-2 transform hover:text-red-500 hover:scale-110">
                          <svg onClick={() => deleteRow(status.Id)}
                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
